...
export function ScheduleGrid({ 
  ...
  return (
    <div className="flex-1 overflow-auto border border-black rounded-lg shadow-lg bg-white relative print:border-none print:shadow-none print:overflow-visible" id="schedule-grid">
        <table className="w-full border-collapse min-w-max md:table-auto print:min-w-0 print:w-full print:table-fixed print:h-full">
        <thead className="bg-slate-100 sticky top-0 z-20 shadow-sm print:static print:bg-white print:shadow-none">
          <tr>
            ...
            <th
              rowSpan={3}
              className="border border-black p-1 md:p-2 text-center min-w-[60px] md:min-w-[90px] sticky left-[70px] md:left-[90px] z-30 bg-slate-100 font-bold text-gray-700 text-xs md:text-lg align-middle print:static print:bg-white print:min-w-0 print:w-[8%]"
            >
              {t.grid.service}
            </th>
            {zones.map((zone, index) => (
              <th
                key={`num-${zone.id}`}
                className="border border-black p-0.5 md:p-1 text-center font-bold text-gray-800 text-xs md:text-base print:text-sm min-w-[80px] md:min-w-0"
              >
                {index + 1}
              </th>
            ))}
          ...
          <tr>
            {zones.map((zone, index) => (
              <th
                key={`name-${zone.id}`}
                className="border border-black p-1 md:p-1.5 text-center font-semibold text-gray-700 align-top group relative print:min-w-0 print:align-middle min-w-[80px] md:min-w-0"
              >
                <div className="print:hidden">
                  <input
                    ...
                    className="w-full text-center bg-transparent font-bold text-gray-900 mb-0.5 border-b border-transparent hover:border-gray-400 focus:border-blue-500 focus:outline-none text-xs md:text-base whitespace-normal leading-tight"
                  />
                  <input
                    type="text"
                    value={zone.contact}
                    onChange={(e) => onZoneUpdate(index, "contact", e.target.value)}
                    className="w-full text-center bg-transparent text-[10px] md:text-sm text-gray-600 font-normal border-b border-transparent hover:border-gray-400 focus:border-blue-500 focus:outline-none leading-tight"
                  />
                </div>
          ...
          <tr>
            {zones.map((zone, index) => (
              <th
                key={`time-${zone.id}`}
                className="border border-black p-1 md:p-1.5 text-center text-xs md:text-sm font-medium text-gray-800 print:text-xs min-w-[80px] md:min-w-0"
              >
                 <input
                  type="text"
                  value={zone.time}
                ...
                >
                  {serviceIndex === 0 && (
                    <td
                      ...
                      className="border border-black p-1.5 md:p-3 font-bold text-gray-700 align-middle sticky left-0 z-10 bg-white text-center print:static print:p-2"
                    >
                      <div className="flex flex-col gap-0.5 md:gap-1">
                        <span className="text-xs md:text-base text-gray-900 print:text-base leading-tight">{dayData.day}</span>
                        <span className="text-[10px] md:text-sm text-gray-500 font-medium print:text-sm whitespace-nowrap leading-tight">{dayData.date}</span>
                      </div>
                    </td>
                  )}

                  <td className="border border-black p-1.5 md:p-3 font-semibold text-gray-700 align-middle sticky left-[70px] md:left-[90px] z-10 bg-inherit text-center text-xs md:text-base print:static print:text-base print:p-2">
                    <span className="print:whitespace-normal print:break-words leading-tight">
                  ...
                  {zones.map((zone) => {
                    ...
                    return (
                      <td
                        key={zone.id}
                        className="border border-black p-0.5 md:p-1 text-center align-middle min-w-[80px] md:min-w-0"
                      >
                         <div className="relative w-full h-full flex items-center justify-center">
                           <input
                             list={`volunteers-${dayIndex}-${serviceIndex}-${zone.id}`}
                             className="w-full h-7 md:h-8 px-1 md:px-2 py-0.5 text-xs md:text-base text-center bg-transparent border-transparent hover:border-slate-300 focus:border-blue-500 focus:outline-none rounded-sm font-medium text-gray-800 print:hidden whitespace-normal overflow-hidden text-ellipsis"
                             value={currentValue || ""}
